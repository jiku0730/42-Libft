...
int main(void)
{
    ...
    /* マクロ TRY を定義して出力 */
    #define TRY(func) if (func(c)) printf(" %s", #func)
    ...
    #undef TRY
    #define TRY(func) if (func(EOF)) { printf(" %s", #func); failures++; }
    TRY(ft_isascii);
    TRYEM;
    putchar('\n');

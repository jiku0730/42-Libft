...
/*   Updated: 2025/10/15 17:10:40 by kjikuhar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	test_add_to_existing_list(t_test *ctx)
{
	...
	assert_ptr_eq(ctx, first, lst);
	assert_str_eq(ctx, "First", (char *)lst->content);
	assert_ptr_eq(ctx, second, lst->next);
	assert_str_eq(ctx, "Second", (char *)lst->next->content);
	assert_null(ctx, lst->next->next);
	free(first);
	free(second);
}
...
static void	test_multiple_adds(t_test *ctx)
{
	...
	assert_ptr_eq(ctx, node1, lst);
	assert_str_eq(ctx, "1", (char *)lst->content);
	assert_str_eq(ctx, "2", (char *)lst->next->content);
	assert_str_eq(ctx, "3", (char *)lst->next->next->content);
